...
void compositor::addTask(renderTask_t *node) {
        if(head == NULL) {
                head = node;
        } else {
...
void compositor::addColor(Color *target, Color *color) {
...
void compositor::renderImage(uint8_t *image, int x, int y, int sizeX, int sizeY) {
        int xreal, yreal;
...
void compositor::renderCharCol(uint8_t ch, Color color, int x, int y) {
...
void compositor::renderText(char *text, Color color, int x, int y, int sizeX, uint skip) {
        int endX = x+sizeX;
...
void compositor::composite() {
        ...
        while(node != NULL) {
                if(node->id == 0) { //Render text
                        ...
                        scroll->skip++;
                        if(scroll->skip == strlen(scroll->text)*6+6) scroll->skip = 0;
